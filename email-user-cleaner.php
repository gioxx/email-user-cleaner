...
function euc_show_duplicates_page(): void {
					...
					<tbody>
						<?php foreach ( $duplicates as $duplicate_group ) : ?>
							<?php foreach ( $duplicate_group['users'] as $user ) : ?>
								...
								$last_login_display = ! empty( $last_login ) ? gmdate( 'Y-m-d H:i:s', strtotime( $last_login ) ) : __( 'Never logged in', 'email-user-cleaner' );
								?>
								<tr>
	...
	if ( isset( $_POST['delete_selected'] ) ) {
		...
		$users_to_delete = isset( $_POST['users_to_delete'] ) ? array_map( 'intval', $_POST['users_to_delete'] ) : array();
...
function euc_find_duplicate_users(): array {
	...
	foreach ( $users as $user ) {
		...
		if ( isset( $email_map[ $user_email ] ) ) {
			...
			$email_map[ $user_email ] = array( $user );
		}
